---
title: "Group Project on Prediction for Credit Card Customer Churners"
author: "Adeoya Adekunle Babafemi, Nnadozie Amadi, Onuchukwu Flora"
date: '2022-12-14'
output:
  html_document:
    toc: no
...
df= subset(Bankchurners, select = -c(CLIENTNUM, 
  Naive_Bayes_Classifier_Attrition_Flag_Card_Category_Contacts_Count_12_mon_Dependent_count_Education_Level_Months_Inactive_12_mon_2,Naive_Bayes_Classifier_Attrition_Flag_Card_Category_Contacts_Count_12_mon_Dependent_count_Education_Level_Months_Inactive_12_mon_1))
df
...
ggplot(qw) +  
geom_point(mapping = aes(x = Avg_Open_To_Buy, y = Total_Trans_Amt)) 
 

# the scatter plot shows no relationship
ggplot(qw) +  
  geom_point(mapping = aes(x = Credit_Limit, y = Total_Trans_Amt)) 
...
ab <- qw %>%
  ggplot( aes(x=Card_Category, y=Credit_Limit, fill=Card_Category)) +
  geom_violin() +
...
# Random Forest
set.seed(123)
fit.rf <- train(Card_Category~., data=qw, method="rf", metric=metric, trControl=control)
